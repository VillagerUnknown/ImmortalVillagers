		...
		UseEntityCallback.EVENT.register(( player, world, hand, entity, hitResult ) -> {
			...
			if( Immortalvillagers.CONFIG.enableVillagerTradesReset && entity.getType().equals( EntityType.VILLAGER ) && player.isCrouching() ) {
				...
				if( professionKey.isPresent() ) {
					...
					if( !NO_RESET_PROFESSIONS.contains( professionKey.get() ) && RESET_ITEMS.contains( itemStack.getItem() ) && itemStack.getItemName().getString().equalsIgnoreCase( RESET_STRING ) ) {
